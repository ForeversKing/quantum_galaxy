...
def cal(N_file, M_file, rankn):
    ...
    # rows: number of gallery images, cols: number of probe images
    rows, cols = similar_mat.shape
    assert rankn <= cols, 'rank n must less than gallery size'

    mask = (np.ones([rows, 1]) * N_id) == (np.ones([cols, 1]) * M_id).T
    arg_sort = np.argsort(similar_mat, axis=1)
    rankn_idx = arg_sort[:, cols-rankn:]
    right_count = 0
    for i in range(rows):
        idx = rankn_idx[i]
        m = mask[i, idx]
...
def arg_parser():
    ...
    parser.add_argument('n', type=int, help='probe feature path')
...
def main():
    args = arg_parser()
    rank1 = cal(args.gallery_feature, args.probe_feature, args.n)
    print('rank1:', rank1)
